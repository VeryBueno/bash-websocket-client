#!/bin/bash
# This script sends a message of up to 127 bytes to a websocket echo server.
...
# Usage : run it, wait for a connection response header, then type away.


# ====== Set the constants below =======

# The websocket server and port
WS_SERVER="echo.websocket.org"
WS_PORT=80
...
nc -l -k -p $WS_LOCAL_PORT | nc $WS_SERVER $WS_PORT &
nc1_pid=$!
nc2_pid=`jobs -p`
...
while read line
do
   # add a newline so that the output doesn't get ugly
   nl=$'\n'
   msg="$line$nl"
